...
## 31. Python 与 PyQt 落地设计

### 31.1 接口分层

当前工程在原有三层控制架构之外，增加了一层 Python 可调用接口，但不改变实时控制核心的职责边界：
...
2. C ABI 适配层：
  - 对外导出稳定的 C 风格函数
  - 暴露轴状态、轴配置、扫描结果和常用控制命令
  - 避免 Python 直接依赖 C++ ABI
3. Python SDK 层：
  - 使用 ctypes 装载 liberob_c_api.so
  - 将 C 结构体转换为 Python dataclass
  - 提供异常风格的调用语义
4. PyQt 控制界面层：
  - 只做交互、展示和任务调度
  - 不直接实现 EtherCAT 业务逻辑

该分层的主要目标是让 Python 层保持易部署，同时避免引入 pybind11 等额外编译依赖。

### 31.2 C ABI 设计原则

新增共享库导出层遵循以下原则：

1. 只暴露稳定、窄接口：初始化、启停、点位运动、随动启停、目标更新、状态查询。
2. 固定大小的数据优先使用 C struct，例如轴元数据和轴状态。
3. 可变长度的数据统一序列化为 JSON 字符串，例如网卡列表、电机列表和绑定报告。
4. Python 侧只依赖 shared library，不需要参与 C++ 模板或 STL 类型绑定。

### 31.3 Python SDK 线程模型

Python SDK 不是实时控制器，只是对 C ABI 的线程安全封装。

设计要点：

1. 所有阻塞型控制命令通过单锁串行进入底层控制器。
2. GUI 刷新读取状态时优先走非阻塞 try-read；若控制命令占用锁，则回退到最近一次缓存状态。
3. 这样可以避免 moveTo 这类等待型调用阻塞界面刷新线程。

换句话说，实时循环依然在 C++ 内部线程中执行，Python 只承担“命令下发”和“状态镜像读取”的角色。
...
这样安排的原因是：高频操作放在轴卡片内部，低频的总线诊断与系统级动作放在外围面板，减少误操作路径。

### 31.5 轮盘随动交互语义

轮盘采用单轴局部 QDial，而不是全局共享控制器，原因如下：

1. 每个轴的角度限位不同，轮盘范围必须与该轴软限位一致。
2. 用户在视觉上能够直接看到该轴当前角度、目标角度和跟随状态。
3. 跟随入口、停止和目标修改放在同一张卡片上，交互闭环更完整。

轮盘的行为约束如下：

1. QDial 值域直接映射到 min_angle_deg 到 max_angle_deg。
2. 轮盘数值变化后不会无节制地逐事件直通 EtherCAT，而是做约 50ms 节流。
3. 只有轴已进入 follow 模式时，轮盘变化才会向底层持续发送 updateFollowTarget。
4. 目标角度在 C++ 轴控制层仍会再次执行 clamp，避免 UI 和控制器边界不一致。

### 31.6 适用边界

该 PyQt 界面定位为上位机调试与运维控制台，适合：
...
因此，后续若需要网络化上位机、ROS 或 Web 控制面板，仍应复用同一 C++ 控制核心与 Python SDK，而不是将逻辑分散到 GUI 代码中。